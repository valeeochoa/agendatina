...
foreach ($cols as $col => $tipo) {
    try { 
        $pdo->query("SELECT $col FROM notificaciones_admin LIMIT 1"); 
    } catch(\Throwable $e) { 
        try { $pdo->exec("ALTER TABLE notificaciones_admin ADD COLUMN $col $tipo"); } catch(\Throwable $ex) {}
    }
}
